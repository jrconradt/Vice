        ...
        OptionRegistry registry)
    {
        ...
        for (int i = 0; i < tokens.Count; i++)
        {
            ...
            if (!registry.TryResolve(token.Value, out var meta))
            {
                ...
                {
                    globals[token.Value] = null;
                }
                else
                {
                    errors.Add($"Unknown option '--{token.Value}'.");
                    globals[token.Value] = null;
                }
                continue;
            }
            ...
            if (meta.TakesValue)
            {
                ...
                if (hasValue)
                {
                    ...
                    globals[meta.CanonicalName] = raw;
                    i++;
                    if (meta.AcceptedValues is not null
                        && !meta.AcceptedValues.Contains(raw, StringComparer.OrdinalIgnoreCase))
                    {
                        errors.Add($"Option '--{meta.CanonicalName}' value '{raw}' is not one of: {string.Join(", ", meta.AcceptedValues)}.");
                    }

                    if (meta.RangeValidator is not null
                        && (!int.TryParse(raw, out var parsed) || !meta.RangeValidator(parsed)))
                    {
                        errors.Add($"Option '--{meta.CanonicalName}' value '{raw}' is out of range.");
                    }
                }
                else
                {
                    errors.Add($"Option '--{meta.CanonicalName}' requires a value.");
                    globals[meta.CanonicalName] = null;
                }
            }
            else
            {
        ...
        for (int i = 0; i < tokens.Count; i++)
        {
            ...
            if (token.Kind == TokenKind.GlobalOption)
            {
                ...
                else
                {
                    ...
                    bool hasValue = !isFlag && i + 1 < tokens.Count && tokens[i + 1].Kind != TokenKind.GlobalOption;

                    if (hasValue)
                    {
                        globals[token.Value] = tokens[i + 1].Value;
                        i++;
                    }
                    else if (needsValue)
                    {
                        errors.Add($"Option '--{token.Value}' requires a value.");
                        globals[token.Value] = null;
                    }
                    else
                    {

...
internal sealed class ArxivSource : IResearchSource
{
                                                            ...
                                                            CancellationToken ct)
    {
        ...
        foreach (var entry in doc.Descendants(Atom + "entry"))
        {
            ...
            var title = ResearchHttp.Collapse(entry.Element(Atom + "title")?.Value);
            var summary = ResearchHttp.Collapse(entry.Element(Atom + "summary")?.Value);
                                              ...
                                              CancellationToken ct)
    {
        ...
        var title = ResearchHttp.Collapse(entry.Element(Atom + "title")?.Value);
        var abstractText = ResearchHttp.Collapse(entry.Element(Atom + "summary")?.Value);

        var authors = entry.Elements(Atom + "author")
            .Select(a => ResearchHttp.Collapse(a.Element(Atom + "name")?.Value))
            .Where(n => n.Length > 0)
            .ToArray();
        ...
        var comment = ResearchHttp.Collapse(entry.Element(ArxivNs + "comment")?.Value);

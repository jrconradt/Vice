...
internal sealed class PubMedSource : IResearchSource
{
    ...
    internal const string Host = "eutils.ncbi.nlm.nih.gov";

    internal static readonly TimeSpan KeylessMinInterval = TimeSpan.FromMilliseconds(1000.0 / 3.0);

    internal static readonly TimeSpan KeyedMinInterval = TimeSpan.FromMilliseconds(1000.0 / 10.0);

    private const string TOOL_NAME = "vice";

    internal const string API_KEY_ENV_VAR = "VICE_NCBI_API_KEY";
                                              ...
                                              CancellationToken ct)
    {
        ...
        var title = ResearchHttp.Collapse(article.Descendants("ArticleTitle").FirstOrDefault()?.Value);
        var abstractText = string.Join("\n",
            article.Descendants("AbstractText").Select(a => ResearchHttp.Collapse(a.Value)).Where(t => t.Length > 0));

        var authors = article.Descendants("Author")
            .Select(a => $"{ResearchHttp.Collapse(a.Element("ForeName")?.Value)} {ResearchHttp.Collapse(a.Element("LastName")?.Value)}".Trim())
            .Where(n => n.Length > 0)
            .ToArray();

        var journal = ResearchHttp.Collapse(article.Descendants("Journal").FirstOrDefault()?.Element("Title")?.Value);
        var pubDate = article.Descendants("PubDate").FirstOrDefault();
        var date = pubDate is null
            ? string.Empty
            : ResearchHttp.Collapse(string.Join(" ", pubDate.Elements().Select(e => e.Value)));

        var meshHeadings = article.Descendants("MeshHeading")
            .Select(m => ResearchHttp.Collapse(m.Element("DescriptorName")?.Value))
            .Where(m => m.Length > 0)
            .ToArray();
                                                     ...
                                                     IViceLogger logger)
    {
        ...
        logger.Log(ViceLogLevel.Debug,
                      $"research source {Name} resolved {id} to {uri.GetLeftPart(UriPartial.Path)}");
    ...
    private static string Identification()
    {
        ...
        var contact = ResearchHttp.ResolveContactEmail();
        if (contact is not null)
        {
            parts.Add($"email={WebUtility.UrlEncode(contact)}");
        }
